...
struct _GtkFileFolderUnix
{
  ...
  gboolean search_magic;
  BeagleQueryResult *query_result;
 ...
 **/
GtkFileSystem *
gtk_file_system_beagle_new (void)
{
...
static GtkFileSystemVolume *
get_search_volume (void)
{
	return (GtkFileSystemVolume *) gtk_file_path_new_dup (SEARCH_MAGIC);
}

static gboolean
is_search_volume (GtkFileSystemVolume *volume)
{
  return ! strcmp (gtk_file_path_get_string ((GtkFilePath *) volume), SEARCH_MAGIC);
}
...
{
	GSList *foo = g_slist_append (NULL, get_root_volume ());
	foo = g_slist_append (foo, get_search_volume ());
	return foo;
}
...
get_search_string (void)
{
  ...
  label = gtk_label_new_with_mnemonic (_("Search For:"));
  ...
  refocus = TRUE;
	...
	results = g_strdup (gtk_entry_get_text (GTK_ENTRY (entry)));
																				 ...
																				 GError              **error)
{
  /* FIXME: set the GError if we can't load the icon */
	if (is_search_volume (volume))
		return get_cached_icon (widget, "gtk-find", pixel_size);
	else
...
gtk_file_folder_unix_finalize (GObject *object)
{
  GtkFileFolderUnix *folder_unix = GTK_FILE_FOLDER_UNIX (object);
	...
	{
    beagle_query_result_free (folder_unix->query_result);
  ...
  GtkFileInfoType types;
	...
	if (folder_unix->search_magic) 
	{
		if (! path)
		{
			...
			str = g_strdup_printf ("Search Results: %s", search_string);
			gtk_file_info_set_display_name (info, str);
			g_free (str);
      ...
      BeagleHit *hit = beagle_query_result_get_by_uri (folder_unix->query_result, uri);
			info = gtk_file_info_new ();
			gtk_file_info_set_display_name (info, filename); /* skip over file:// */
			gtk_file_info_set_is_hidden (info, FALSE);
...
static void
beagle_query_result_list_children (BeagleQueryResult *bqr, GSList **children)
{
  GSList *all;

  for (all = beagle_query_result_get_all (bqr); all != NULL; all = all->next) {
    BeagleHit *hit = all->data;

    /* Filter out non-file:// Uris */
    if (! strncmp (beagle_hit_get_uri (hit), "file://", 7)) {
      *children = g_slist_prepend (*children, filename_to_path (beagle_hit_get_uri (hit) + 7));
    }
  }
}
